contract('Power', function(accounts) {
  ...
  it("should allow to power up and power down.", function(done) {
    ...
    Token.new(accounts[0], 2, power.address).then(function(contract) {
      ...
      assert.equal(bal.toNumber(), 50, 'power up failed');
      return power.totalSupply.call();
	}).then(function(total) {
      assert.equal(total.toNumber(), 150, 'economy failed, 1000 power expected.');
      return token.issue(1000);
    }).then(function() {
      ...
      return power.down(50);
    }).then(function() {
      return power.balanceOf.call(accounts[0]);
	}).then(function(bal) {
      assert.equal(bal.toNumber(), 0, 'power down failed');
      return token.balanceOf.call(accounts[0]);
	}).then(function(bal) {
      assert.equal(bal.toNumber(), 1500, 'power down failed');
