...
contract('SnGTable', function(accounts) {
  ...
  it("should join table, then settle, then leave.", async () => {
    const table = await Table.new(ORACLE, babz(2), 2, 0);
    ...
    const settleReceipt = new Receipt(table.address).settle(1, 3, [babz(9), babz(-10)]).sign(ORACLE_PRIV);
    ...
    assert.equal(lhn.toNumber(), 3, 'settlement failed for last hand.');3
    assert.equal(oracleBalAfter.sub(oracleBalBefore).toNumber(), 1000000000000, 'withdraw rake failed.');

    assert.equal(seat1[1].toNumber(), 109000000000000, 'settlement failed for seat pos 2.');
    assert.equal(balancePlayer1After.sub(balancePlayer1Before).toNumber(), 0, 'settlement failed for seat pos 2.');
...
it("should join table, then settle, then leave broke.", async () => {
    const table = await Table.new(ORACLE, babz(1), 2, 0);
    ...
    assert.equal(seat[3].toNumber(), 3, 'leave request failed.');
    // prepare settlement
    var settleReceipt = new Receipt(table.address).settle(1, 3, [babz(39.2), babz(-40)]).sign(ORACLE_PRIV);
    ...
    assert.equal(balanceAfter.sub(balanceBefore).toNumber(), 0);
  });

  it('should join table, then net, then leave.', async () => {

    const table = await Table.new(ORACLE, babz(20), 2, 0);
